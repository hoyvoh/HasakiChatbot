...
def get_decision(query):
    ...
    (3) Nếu người dùng nhắc đến việc tìm sản phẩm theo yêu cầu:
        hãy trả về theo cú pháp: {'signal':'3','product_term':'<mô tả sản phẩm>','price':'<price>','rating':'<rating>','combo':'<thông tin khuyến mãi>','others':'<thông tin khác>'}
        chú ý, <price> phải là một con số hoặc một string có thể chuyển thành int một cách trực tiếp.
